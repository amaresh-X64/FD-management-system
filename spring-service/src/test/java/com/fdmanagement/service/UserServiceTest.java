...
class UserServiceTest {
	...
	class CreateUser {
		...
		void shouldReturnMappedResponse_whenInputIsValidUserWithUniqueEmail() {
			CreateUserRequest req = new CreateUserRequest(
					"Amaresh", "lakshman@gmail.com",
					new BigDecimal("80000"), new BigDecimal("40000"));

			when(userRepository.findByEmail("lakshman@gmail.com")).thenReturn(Optional.empty());
			when(userRepository.save(argThat(u -> "lakshman@gmail.com".equals(u.getEmail()))))
			...
			verify(userRepository).save(argThat(u -> "lakshman@gmail.com".equals(u.getEmail())));
		}

		@Test
		void shouldReturnMappedResponse_whenInputIsUserWithZeroExpenses() {
			User zeroExpUser = User.builder().id(2L).name("Akhilesh")
					.email("ram@gmail.com")
					.monthlyIncome(new BigDecimal("50000"))
					.monthlyExpenses(new BigDecimal("0")).build();
			UserResponse zeroExpResponse = new UserResponse(
					2L, "Akhilesh", "ram@gmail.com",
					new BigDecimal("50000"), new BigDecimal("0"), null);
			CreateUserRequest req = new CreateUserRequest(
					"Akhilesh", "ram@gmail.com",
					new BigDecimal("50000"), new BigDecimal("0"));

			when(userRepository.findByEmail("ram@gmail.com")).thenReturn(Optional.empty());
			when(userRepository.save(argThat(u -> "ram@gmail.com".equals(u.getEmail()))))
		...
		void shouldMapAllRequestFieldsOntoEntity_whenInputIsValidRequest() {
			...
			verify(userRepository).save(argThat(u -> "Amaresh".equals(u.getName()) &&
					"lakshman@gmail.com".equals(u.getEmail()) &&
		...
		void shouldThrowRuntimeExceptionAndNeverSave_whenInputIsDuplicateEmail() {
			CreateUserRequest req = new CreateUserRequest(
					"Any Name", "lakshman@gmail.com",
					new BigDecimal("80000"), new BigDecimal("40000"));

			when(userRepository.findByEmail("lakshman@gmail.com")).thenReturn(Optional.of(savedUser));

			assertThatThrownBy(() -> userService.createUser(req))
					.isInstanceOf(RuntimeException.class)
					.hasMessageContaining("Email already registered: lakshman@gmail.com");
		...
		void shouldThrowRuntimeExceptionWithCorrectEmail_whenInputIsAnotherDuplicateEmail() {
			User priya = User.builder().id(2L).name("Priya").email("ram@gmail.com")
					.monthlyIncome(BigDecimal.ONE).monthlyExpenses(BigDecimal.ONE).build();
			CreateUserRequest req = new CreateUserRequest(
					"Any Name", "ram@gmail.com",
					new BigDecimal("80000"), new BigDecimal("40000"));

			when(userRepository.findByEmail("ram@gmail.com")).thenReturn(Optional.of(priya));

			assertThatThrownBy(() -> userService.createUser(req))
					.isInstanceOf(RuntimeException.class)
					.hasMessageContaining("Email already registered: ram@gmail.com");
		}
	}
	...
	class GetUser {
		...
		void shouldReturnMappedResponse_whenInputIsExistingUserId42() {
			User user42 = User.builder().id(42L).name("Amaresh")
					.email("lakshman@gmail.com")
					.monthlyIncome(new BigDecimal("80000"))
					.monthlyExpenses(new BigDecimal("40000")).build();
			UserResponse response42 = new UserResponse(
					42L, "Amaresh", "lakshman@gmail.com",
					new BigDecimal("80000"), new BigDecimal("40000"), null);
